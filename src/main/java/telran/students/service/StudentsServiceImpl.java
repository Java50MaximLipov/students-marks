...
public class StudentsServiceImpl implements StudentsService {
	...
	private List<Mark> getStudentMarks(long id, MatchOperation matchMarks) {
		...
		ProjectionOperation projectOperation = Aggregation.project(MARKS_SUBJECT_FIELD, MARKS_SCORE_FIELD,
				MARKS_DATE_FIELD);
		Aggregation pipeline = Aggregation.newAggregation(matchStudentOperation, unwindOperation, matchMarks,
				projectOperation);
		...
		List<Mark> res = documents.stream().map(d -> new Mark(d.getString("subject"), d.getInteger(SCORE_FIELD),
				d.getDate(DATE_FIELD).toInstant().atZone(ZoneId.systemDefault()).toLocalDate())).toList();
		return res;
	}
	...
	public List<StudentAvgScore> getStudentsAvgScoreGreater(int avgThreshold) {
		...
		MatchOperation matchOperation = Aggregation.match(Criteria.where(AVG_SCORE_FIELD).gt(avgThreshold));
	...
	public List<Long> getBestStudents(int nStudents) {
		...
		MatchOperation matchOperation = Aggregation
				.match(Criteria.where(MARKS_SCORE_FIELD).gt(BEST_STUDENTS_MARK_THRESHOLD));
	...
	public List<Long> getWorstStudents(int nStudents) {
		...
		ProjectionOperation projectionOperation = Aggregation.project(ID_FIELD).and(expression).as(SUM_SCORES_FIELD);
		...
		List<Long> res = mongoTemplate.aggregate(pipeLine, StudentDoc.class, Document.class).getMappedResults().stream()
				.map(d -> d.getLong(ID_FIELD)).toList();
